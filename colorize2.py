...
def create_field( values, ifield, nb_layers, prolong_field = False ):
    ...
    """
    assert(ifield >= 0)
    assert(ifield < values.shape[-1]), f"field {values.shape[-1]} out of bounds = {ifield}"
...
if __name__ == '__main__':
    ...
    rank = comm.Get_rank()
    size = comm.Get_size()
    if rank == 0: print("Nombre de coeurs disponibles : ", size)
    ...
    if rank == 0:
        ...
        # DISTRIBUTION AUX COEURS
        n_rows_per_process = values_gray.shape[0] // size
        start_idx = lambda i: i*n_rows_per_process
        end_idx = lambda i: (i+1)*n_rows_per_process if i != size-1 else values_gray.shape[0]
        
        intensity = (1./255.)*create_field(values_gray, INTENSITY, nb_layers=2, prolong_field=True)
        intensity_to_send = [intensity[start_idx(i):end_idx(i)] for i in range(size)]
        # intensity = comm.Scatter(intensity_to_send, root=0)
        # all_values_gray_local = [values_gray[start_idx(i):end_idx(i)] for i in range(size)]
        # all_val_ycbcr = [val_ycbcr[start_idx(i):end_idx(i)] for i in range(size)]
        # all_val_hsv = [val_hsv[start_idx(i):end_idx(i)] for i in range(size)]

        intensity_local = intensity_to_send[0]
        for dst_rk in range(1, size):
            # message = [all_values_gray_local[dst_rk], all_val_ycbcr[dst_rk], all_val_hsv[dst_rk]]
            message = intensity_to_send[dst_rk]
            comm.send(message, dest=dst_rk, tag=1)
        # values_gray_local, val_ycbcr_local, val_hsv_local = all_values_gray_local[0], all_val_ycbcr[0], all_val_hsv[0]

    if rank != 0:
        intensity_local = comm.recv(source=0, tag=1)
        # comm.Recv(values_gray_local, source=0)
        # values_gray_local, val_ycbcr_local, val_hsv_local = comm.recv(source=0, tag=1)
    # intensity = (1./255.)*create_field(values_gray, INTENSITY, nb_layers=2, prolong_field=True)
    # intensity_local = (1./255.)*create_field(values_gray_local, INTENSITY, nb_layers=2, prolong_field=True)
    # comm.Scatter(intensity_to_send, intensity, root=0)

    print(f"On rank {rank}, intensity shape : {intensity_local.shape}")
    ...
    means_local = compute_means(intensity_local)
    end = time.time() - deb
    # print(f"Temps calcul moyenne : {end} secondes")
    print(f"Temps calcul moyenne local on rank {rank} : {end} secondes")
    ...
    variance_local = compute_variance(intensity_local, means_local)
    end = time.time() - deb
    # print(f"Temps calcul variance : {end} secondes")
    print(f"Temps calcul variance local on rank {rank} : {end} secondes")

    # Calcul de la matrice utilisee pour minimiser la fonction quadratique
    deb = time.time()
    if rank == 0:
        means, variance = [means_local], [variance_local]
        for src_rk in range(1, size):
            means_local, variance_local = comm.recv(source=src_rk, tag=2)
            means.append(means_local)
            variance.append(variance_local)
        means = np.concatenate(means, axis=0)
        variance = np.concatenate(variance, axis=0)
        A = compute_matrix((means.shape[1]-2,means.shape[0]-2), 0, intensity, means, variance)  
        
        for dst_rk in range(1, size):
            comm.send(A, dest=dst_rk, tag=3)
    else:
        comm.send([means_local, variance_local], dest=0, tag=2)
        A = comm.recv(source=0, tag=3)
    # A = compute_matrix((means.shape[1]-2,means.shape[0]-2), 0, intensity, means, variance)
    # A_local = compute_matrix((means_local.shape[1]-2,means_local.shape[0]-2), 0, intensity_local, means_local, variance_local)
    end = time.time() - deb
    # print(f"Temps calcul matrice : {end} secondes")
    print(f"Temps calcul matrice local on rank {rank} : {end} secondes")

    ### METTRE EN AVANCE AVEC ROOT 0 ###
    # Calcul des seconds membres
    # im = Image.open(marked_img)
    # im_ycbcr = im.convert('YCbCr')
    # val_ycbcr = np.array(im_ycbcr)
    ...
    # Cb = (1./255.)*np.array(val_ycbcr[:,:,CB].flat, dtype=np.double)
    # Cr = (1./255.)*np.array(val_ycbcr[:,:,CR].flat, dtype=np.double)
    Cb_local = (1./255.)*np.array(val_ycbcr_local[:,:,CB].flat, dtype=np.double)
    Cr_local = (1./255.)*np.array(val_ycbcr_local[:,:,CR].flat, dtype=np.double)

    deb=time.time()
    # b_Cb = -A.dot(Cb)
    # b_Cr = -A.dot(Cr)
    b_Cb_local = -A_local.dot(Cb_local)
    b_Cr_local = -A_local.dot(Cr_local)
    end = time.time() - deb
    # print(f"Temps calcul des deux seconds membres : {end} secondes")
    print(f"Temps calcul des deux seconds membres local on rank {rank} : {end} secondes")

    # im_hsv = im.convert("HSV")
    # val_hsv = np.array(im_hsv)
    deb = time.time()
    # fix_coul_indices = search_fixed_colored_pixels(val_hsv)
    fix_coul_indices_local = search_fixed_colored_pixels(val_hsv_local)
    end = time.time() - deb
    # print(f"Temps recherche couleur fixee : {end} secondes")
    print(f"Temps recherche couleur fixee local on rank {rank} : {end} secondes")

    # Application de la condition de Dirichlet sur la matrice :    
    deb = time.time()
    # apply_dirichlet(A, fix_coul_indices)
    apply_dirichlet(A_local, fix_coul_indices_local)
    end = time.time() - deb
    # print(f"Temps application dirichlet sur matrice : {end} secondes")
    print(f"Temps application dirichlet sur matrice local on rank {rank} : {end} secondes")
    ...
    x0_local = np.zeros(Cb_local.shape,dtype=np.double)
    new_Cb_local = Cb_local + minimize(A_local, b_Cb_local, x0_local, niters, epsilon)
    end = time.time() - deb
    # print(f"\nTemps calcul min Cb : {time.time()-deb}")
    print(f"Temps calcul min Cb local on rank {rank} : {end} secondes")
    ...
    x0_local = np.zeros(Cr_local.shape,dtype=np.double)
    new_Cr_local = Cr_local + minimize(A_local, b_Cr_local, x0_local, niters, epsilon)
    end = time.time() - deb
    print(f"Temps calcul min Cr local on rank {rank} : {end} secondes")
    ...
    shape = (means_local.shape[0]-2,means_local.shape[1]-2)
    new_image_array_local = np.empty((shape[0],shape[1],3), dtype=np.uint8)
    new_image_array_local[:,:,0] = intensity_local[2:-2,2:-2].astype('uint8')
    new_image_array_local[:,:,1] = np.reshape(new_Cb_local, shape).astype('uint8')
    new_image_array_local[:,:,2] = np.reshape(new_Cr_local, shape).astype('uint8')
    if rank == 0:
        new_image_parts = [new_image_array_local]
        for i in range(1, size):
            new_image_parts.append(comm.recv(source=i, tag=3))
        new_image_array = np.concatenate(new_image_parts, axis=0)
        new_im = Image.fromarray(new_image_array, mode='YCbCr')
        new_im.convert('RGB').save(output, 'PNG')
    else:
        comm.send(new_image_array_local, dest=0, tag=3)
